...
export default function EditBuildingScreen(){
    ...
    const {
        ...
        isLoading: fetchedBuildingByIdIsLoading,
        refetch: fetchedBuildingByIdRefetch,
    } = useGetBuildingByIdQuery(buildingId)
    // console.log('fetchedBuildingById: ', fetchedBuildingById)

    const [updateBuilding, {isLoading: updateBuildingIsLoading}] = useUpdateBuildingMutation();

    useEffect(() => {
        if(fetchedBuildingById){
            setForm(fetchedBuildingById);
        }
    }, [fetchedBuildingById]);
    ...
    return(
        <div>
            <div className="bg-gray-50 rounded shadow">
                ...
                {
                    fetchedBuildingByIdIsLoading ? (
                            <div  className="text-center py-4">
                                <Spinner/>
                            </div>
                    ) : fetchedBuildingByIdError ? (
                            <div className="text-center py-4">
                                <Alert
                                    color="red"
                                    message="Error"
